...
public class ProductTaxController {
	...
	@RequestMapping(value="/tax/excel_test.do", method=RequestMethod.GET)
	public String productInfoExcelInsert() {
		...
		try {
			...
			FileInputStream fis= new FileInputStream("C:\\Users\\3bgogi\\Desktop\\3공유\\300_당일주문서\\tax.xlsx");
			...
			// 분기별로 처리해서 해야함
					...
					for(int rowindex = rowStartIndex; rowindex<rows;rowindex++){
						...
						ProductInfoVO piVO = null;
						
						if(row !=null){
							piVO = new ProductInfoVO();
							...
							for(columnindex=0;columnindex<12;columnindex++){
								...
								if(cell==null) {
									...
									if(columnindex==0) {
										piVO.setRcFk((int)cell.getNumericCellValue());
											
									}if(columnindex==1) {
										piVO.setPiName(cell.getStringCellValue()+"");
											
									}if(columnindex==2) {
										
										piVO.setPiQty((int)cell.getNumericCellValue());
										
									}if(columnindex==3) {
										
										piVO.setPiMeasure(cellTypeReturn(cell));
											
									}if(columnindex==4) {
										piVO.setPiCost((int)cell.getNumericCellValue());
											
									}if(columnindex==5) {
										piVO.setPiTax((int)cell.getNumericCellValue());
										
									}if(columnindex==6) {
										if(cell != null) {
											piVO.setPiTotalCost((int)cell.getNumericCellValue());
											
										}else {
											piVO.setPiTotalCost(0);
										}
											
									}if(columnindex==8) {
										
										if((int)cell.getNumericCellValue() == 1) {
											piVO.setPiTaxbilCheckFlag(true);
											
										}else {
											piVO.setPiTaxbilCheckFlag(false);
										...
										if(HSSFDateUtil.isCellDateFormatted(cell)) {
											piVO.setPiInputDate(cellTypeReturn(cell));
							           		
							           	}else {		                            		
							           		piVO.setPiInputDate(cell.getNumericCellValue()+"");
							           	}
							...
							if(piVO.getPiName() != null) {							
								ptService.insertProductInfo(piVO);
